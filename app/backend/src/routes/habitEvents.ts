...
router.post('/bulk', authenticateToken, HabitEventController.bulkLogEvents);

// POST /api/habit-events/:habitId - Log a new habit event (completion, skip, or relapse)
router.post('/:habitId', authenticateToken, validateUUID('habitId'), HabitEventController.logEvent);

// GET /api/habit-events/today - Get today's events for dashboard
router.get('/today', authenticateToken, HabitEventController.getTodayEvents);

// GET /api/habit-events/calendar - Get completion calendar for date range
router.get('/calendar', authenticateToken, HabitEventController.getCompletionCalendar);

// GET /api/habit-events/stats - Get event statistics (all habits or specific habit)
router.get('/stats', authenticateToken, HabitEventController.getEventStatistics);

// GET /api/habit-events/all - Get all events for a user with filtering and pagination
router.get('/all', authenticateToken, HabitEventController.getAllUserEvents);

// GET /api/habit-events/:habitId/events - Get habit events with filtering and pagination
router.get('/:habitId/events', authenticateToken, validateUUID('habitId'), HabitEventController.getHabitEvents);

// GET /api/habit-events/event/:eventId - Get a specific habit event by ID
router.get('/event/:eventId', authenticateToken, validateUUID('eventId'), HabitEventController.getEventById);

// PUT /api/habit-events/event/:eventId - Update a habit event
router.put('/event/:eventId', authenticateToken, validateUUID('eventId'), HabitEventController.updateEvent);

// DELETE /api/habit-events/event/:eventId - Delete a habit event
router.delete('/event/:eventId', authenticateToken, validateUUID('eventId'), HabitEventController.deleteEvent);
